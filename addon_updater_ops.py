...
class addon_updater_install_popup(bpy.types.Operator):
	...
	def draw(self, context):
		...
		if updater.update_ready:
			col.label(text="Update {} ready!".format(str(updater.update_version)), icon="LOOP_FORWARDS")
...
class addon_updater_update_target(bpy.types.Operator):
	...
	def poll(cls, context):
		print(updater.update_ready, len(updater.tags))
		if updater.invalidupdater:
			return False
...
class addon_updater_restore_backup(bpy.types.Operator):
	...
	def poll(cls, context):
		try:
			return isdir(join(updater.stage_path,"backup"))
		except:
			return False
...
def background_update_callback(update_ready):
	...
	if not update_ready:
		return
	if updater_run_install_popup_handler not in update_post_handler and not ran_autocheck_install_popup:
		update_post_handler.append(updater_run_install_popup_handler)
		ran_autocheck_install_popup = True
...
def show_reload_popup():
	...
	if saved_state != None and "just_updated" in saved_state and saved_state["just_updated"]:
		...
		if updater_run_success_popup_handler not in update_post_handler and not ran_update_sucess_popup:
			update_post_handler.append(updater_run_success_popup_handler)
			ran_update_sucess_popup = True
...
def update_settings_ui(self, context, element=None, condensed=False):
	...
	if updater.error is not None:
		...
		split = subcol.split(align=True)
		split.scale_y = 2
		split.operator(addon_updater_update_now.bl_idname, text="Update directly to " + str(updater.include_branch_list[0]))
		split = subcol.split(align=True)
		split.scale_y = 2
		split.operator(addon_updater_check_now.bl_idname, text="", icon="FILE_REFRESH")
	...
	if condensed:
		...
		if updater.include_branches and len(updater.include_branch_list) > 0:
			...
			col.operator(addon_updater_update_target.bl_idname, text="Install latest {} / old version".format(branch))
		else:
			col.operator(addon_updater_update_target.bl_idname, text="Reinstall / install old version")
		lastdate = "none found"
		backuppath = join(updater.stage_path,"backup")
		if "backup_date" in updater.json and isdir(backuppath):
...
def update_settings_ui_condensed(self, context, element=None):
	update_settings_ui(self, context, element=element, condensed=True)
...
def register(bl_info):
	...
	updater.website = "https://www.blendermarket.com/products/bricker"
